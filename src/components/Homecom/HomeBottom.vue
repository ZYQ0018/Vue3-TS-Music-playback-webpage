...
<template>
    ...
    <div class="show" :style="{ backgroundImage: 'url(' + playList[playIndex].al.picUrl + ')' }" v-show="isshowtab" @mouseenter="showTab" @mouseleave="closTab">
        <div class="show-left"><img style="width: 140px; height: 140px;" :src="playList[playIndex].al.picUrl" title="展开歌曲"/></div>
    ...
    <div class="container">
        <div class="left" @mouseenter="showTab" @mouseleave="closTab">
            ...
            <div>
                <h4 style="cursor:default; margin-bottom: 5px;">{{ playList[playIndex].name}}</h4>
                <span style="cursor:default; margin-right: 15px; font-size: 14px; font-weight: 400;" v-for="(arname) in playList[playIndex].ar" :key="arname.id">{{arname.name}}</span>
            </div>
        ...
        <div class="center">
            ...
            <div class="bottom">
               <div>{{ convertSeconds(currentTime) || '00:00' }}</div><div class="Progress"><div class="Progress-childer" :style="{ width: (1016 /  duration * currentTime) + 'px' || 0 }"></div></div><div>{{ convertSeconds(duration) || '00:00' }}</div>
            </div>
        </div>
        <div class="right">
