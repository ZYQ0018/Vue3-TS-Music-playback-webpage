    ...
    <div class="container">
        <div class="div" v-for="(item,index) in pops.dayMusic" :key="item.id" @click="playAudio(item,index)">
            <div class="div-left">
                ...
                <span class="details">
                <span style="font-size: 15px; letter-spacing: 0.2px;">{{item.name}}</span>
                <span style="display: flex;">
                    <span style="font-size: 13px; color: #6d6a6a; margin-right:10px; letter-spacing: 0.2px;" v-for="(namelist) in item.ar" :key="namelist.id">{{ namelist.name }}</span>
                </span>
